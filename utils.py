...
#检查变量取值信息
def check_feature_value_counts(df):
    sig_features = [ ]
    bin_features = [ ]
    muti_features = [ ]
    ...
    col.remove('certId')
    for c in col:
        cnt = df[c].value_counts().shape[0]
        if cnt < 2:
            sig_features.append(c)
        elif cnt < 4 :
            bin_features.append(c)
        elif cnt >=4:
            muti_features.append(c)
            
    print("单值变量：{}\n".format(sig_features))
    print("二值变量：{}\n".format(bin_features))
    print("多值变量：{}\n".format(muti_features))
...
def getbankinfo(cardbin):
    key = CARDBIN.get(cardbin[:6]) if CARDBIN.get(cardbin[:6]) else CARDBIN.get(cardbin[:4], None)
    if key:
        key = key.split('|')
        #print("bank:{}".format(key[0]))
        #print("cardname:{}".format(key[1]))
        #print("cardtype:{}".format(key[2]))
        return key[0],key[1],key[2]
    else:
        print('其他银行')
        return '其他银行','其他卡产品','其他卡类型'

def getbankname(cardbin):
    key = CARDBIN.get(cardbin[:6]) if CARDBIN.get(cardbin[:6]) else CARDBIN.get(cardbin[:4], None)
    if key:
        key = key.split('|')
        return key[0]
...
def encoding_bankcard(df):
    df['bankCard'] = df['bankCard'].astype(int) #去除标点符号
    df['bankCard'] = df['bankCard'].astype(str) #转为字符串
    df['bankCard'] = df['bankCard'].apply(lambda x: x[0:6])#获取卡BIN号
    df['bankCard'] = df.bankCard.apply(lambda x: getbankinfo(x))#获取银行名称
    #bankname = pd.get_dummies(df['bankCard'],prefix="bankCard",drop_first=True)#进行 One-hot编码
    #df.drop(['bankCard'],axis=1,inplace=True)
    #df = pd.concat([df,bankname],axis=1)
    return df
...
"356868":"平安银行（深圳发展银行）股份有限公司|信用卡|贷记卡" ,
"356869":"平安银行（深圳发展银行）股份有限公司|信用卡|贷记卡" ,
...
"412962":"平安银行（深圳发展银行）（借记卡）|发展借记卡|借记卡" ,
"412963":"平安银行（深圳发展银行）（借记卡）|发展借记卡|借记卡" ,
"413520":"中国工商银行||" ,
"415599":"民生银行|民生借记卡|借记卡" ,
"415752":"平安银行（深圳发展银行）（借记卡）|国际借记卡|借记卡" ,
"415753":"平安银行（深圳发展银行）（借记卡）|国际借记卡|借记卡" ,
...
"435744":"平安银行（深圳发展银行）股份有限公司|沃尔玛百分卡|贷记卡" ,
"435745":"平安银行（深圳发展银行）股份有限公司|沃尔玛百分卡|贷记卡" ,
...
"483536":"平安银行（深圳发展银行）股份有限公司|VISA白金卡|贷记卡" ,
...
"526855":"平安银行（深圳发展银行）股份有限公司|平安银行（深圳发展银行）信用卡|贷记卡" ,
"527414":"兴业银行|加菲猫信用卡|贷记卡" ,
"528020":"平安银行（深圳发展银行）股份有限公司|平安银行（深圳发展银行）信用卡|贷记卡" ,
...
"531659":"平安银行（深圳发展银行）股份有限公司|白金信用卡|贷记卡" ,
...
"620010":"平安银行（深圳发展银行）（借记卡）|公益预付卡|预付费卡" ,
"620011":"BC卡公司（26630410）|BC|CUPGiftCard" ,
...
"620103":"BC卡公司（26630410）|BC|CUPGiftCard" ,
"620105":"The||" ,
"620106":"BC卡公司（26630410）|BC|CUPGiftCard" ,
...
"620120":"BC卡公司（26630410）|BC|CUPGiftCard" ,
"620123":"BC卡公司（26630410）|BC|CUPGiftCard" ,
"620124":"中国工商银行澳门分行|预付卡|预付费卡" ,
"620125":"BC卡公司（26630410）|BC|CUPGiftCard" ,
"620126":"澳门通股份有限公司（28020446）|旅游卡|预付费卡" ,
"620129":"The||" ,
"620132":"BC卡公司（26630410）|BC|CUPGiftCard" ,
...
"620220":"BC卡公司（26630410）|BC|CUPGiftCard" ,
"620278":"BC卡公司（26630410）|BC|CUPGiftCard" ,
...
"620812":"BC卡公司（26630410）|BC|CUPGiftCard" ,
...
"621040":"莫斯科人民储蓄银行（26690643）|cup|unioncard" ,
...
"621049":"蒙古郭勒姆特银行（26620496）|Golomt|" ,
...
"622155":"平安银行（深圳发展银行）股份有限公司|平安银行（深圳发展银行）信用卡|贷记卡" ,
"622156":"平安银行（深圳发展银行）股份有限公司|平安银行（深圳发展银行）信用卡|贷记卡" ,
"622157":"平安银行（深圳发展银行）股份有限公司|白金信用卡|贷记卡" ,
...
"622525":"平安银行（深圳发展银行）股份有限公司|人民币信用卡金卡|贷记卡" ,
"622526":"平安银行（深圳发展银行）股份有限公司|人民币信用卡普卡|贷记卡" ,
"622531":"天津农村商业银行|吉祥商联IC卡|借记卡" ,
"622532":"晋城银行股份有限公司|珠联璧合卡|借记卡" ,
"622535":"平安银行（深圳发展银行）（借记卡）|聚财卡金卡|借记卡" ,
"622536":"平安银行（深圳发展银行）（借记卡）|聚财卡VIP金卡|借记卡" ,
"622538":"平安银行（深圳发展银行）（借记卡）|发展卡（银联卡）|借记卡" ,
"622539":"平安银行（深圳发展银行）（借记卡）|聚财卡白金卡和钻石卡|借记卡" ,
...
"622889":"中国工商银行（亚洲）有限公司|ICBC（Asia）|" ,
...
"622983":"平安银行（深圳发展银行）（借记卡）|聚财卡钻石卡|借记卡" ,
"622985":"上海银行|申卡社保副卡（有折）|借记卡" ,
"622986":"平安银行（深圳发展银行）|平安银行（深圳发展银行）借记卡|借记卡" ,
"622987":"上海银行|申卡社保副卡（无折）|借记卡" ,
"622988":"建设银行|理财金卡|借记卡" ,
"622989":"平安银行（深圳发展银行）|平安银行（深圳发展银行）借记卡|借记卡" ,
...
"623058":"平安银行（深圳发展银行）|平安银行（深圳发展银行）IC借记卡|借记卡" ,
...
"623323":"NongHyup||" ,
"623324":"PVB||" ,
"623328":"中信嘉华银行有限公司（25160344）|CNCBI|" ,
"623331":"State||" ,
"623334":"K||" ,
...
"625136":"中国银行（香港）（25190344）|中银Good|" ,
...
"625360":"平安银行（深圳发展银行）股份有限公司|平安中国旅游信用卡|贷记卡" ,
"625361":"平安银行（深圳发展银行）股份有限公司|平安中国旅游白金信用卡|贷记卡" ,
...
"625823":"平安银行（深圳发展银行）股份有限公司|贷记IC卡|贷记卡" ,
"625825":"平安银行（深圳发展银行）股份有限公司|白金IC卡|贷记卡" ,
...
"627066":"平安银行（深圳发展银行）股份有限公司|一账通借贷合一卡普卡|借记卡" ,
"627067":"平安银行（深圳发展银行）股份有限公司|一账通借贷合一卡金卡|借记卡" ,
"627068":"平安银行（深圳发展银行）股份有限公司|一账通借贷合一白金卡|借记卡" ,
"627069":"平安银行（深圳发展银行）股份有限公司|一账通借贷合一钻石卡|借记卡" ,
...
"628296":"平安银行（深圳发展银行）股份有限公司|公务卡|贷记卡" ,
...
"998800":"平安银行（深圳发展银行）（借记卡）|发展借记卡（银联卡）|借记卡" ,
"998801":"平安银行（深圳发展银行）股份有限公司|发展信用卡（银联卡）|贷记卡" ,
"998802":"平安银行（深圳发展银行）股份有限公司|发展信用卡（银联卡）|贷记卡" ,
